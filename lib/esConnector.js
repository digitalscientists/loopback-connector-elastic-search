...
ESConnector.prototype.discoverProperties = function(model){
  var self = this;
  var model_options = self._models[model].settings[self.name];
  self.elasticServer.get({
    ...
    function (response) {
      ...
      if( !self.utils.isEmpty(response) ) {

        var index_name_in_es = Object.keys(response)[0];
        var properties = response[index_name_in_es]['mappings'][model_options.type].properties;
        /** Transforms "ES _mapping properties" into "StrongLoop model properties" **/
        Object
          .keys(properties)
          .forEach(
            function(property){
              Object.keys(properties[property]).forEach(
                function(attr){
                  if( attr != "type" ){
                    delete properties[property][attr];
                  }
                }
              );
            }
          );
        self._models[model].properties = properties;
        if( !self._models[model].properties["_id"] ) {
          self._models[model].properties["_id"] = { "type": "string", "id": true };
        }
      } else {
        self._models[model].properties = {};
      }
...
ESConnector.prototype.all = function all(model, filter, done) {
  var self = this;
  var model_options = self._models[model].settings[self.name];
  self.searchType = model_options.type ? model_options.type : "";
...
/**
 * Return a promise which will resolve to the
 *  result of a _search operation
 * @param {String} model name
 * @param {String} filter criteria
 */
ESConnector.prototype.rawSearch = function count(model, filter) {
  var self = this;
  var model_options = self._models[model].settings[self.name];
  self.searchType = model_options.type ? model_options.type : "";
  return self.elasticServer.search(
    self.makeFilter(filter, null, null)
  );
};
...
ESConnector.prototype.create = function (model, data, done) {
  ...
  if( !self.utils.isEmpty(self._models[model].properties) ) {
...
ESConnector.prototype.save = function (model, data, done) {
  ...
  var document = self.makeFilterIndexAndType();
  document["id"] = data[self.idField];
  document["body"] = {};
  if( !self.utils.isEmpty(self._models[model].properties) ) {
    document["body"]["doc"] = self.matchDataToModel(model, data);
  } else {
    document["body"]["doc"] = data;
  }
